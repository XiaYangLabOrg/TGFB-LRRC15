...
method <- 'deg_0.5' # hdWGCNA_M, hdWGCNA_all, deg_0$, degree
...
p <- FeaturePlot(scing_obj, features=c(common_degs_dir), combine = T, split.by = 'TGFB_inducible',label = T, slot = 'new_scale.data') &
  theme(legend.position="right") &
  # scale_color_gradient2(low="blue",mid="white", high = "red")
  scale_colour_gradientn(limits=c(-4,4),colors = rev(brewer.pal(n = 11, name = "RdBu")))
save_plot(paste0("../scRNAseq_analysis/module_umaps/SCING_module_umaps_singlecells",outfile_suffix,".pdf"), p, base_width = 20, base_height=20)

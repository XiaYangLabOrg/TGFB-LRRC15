...
method <- 'hdWGCNA_all' # hdWGCNA_M, hdWGCNA_all, deg_0$, degree
...
p <- FeaturePlot(scing_metacell, features=c(common_degs_dir,'SCING-M0-hdWGCNA-all'), combine = T,label = F, reduction = 'pca', split.by = 'TGFB_inducible', slot = 'new_scale.data') &
...
p <- FeaturePlot(scing_obj, features=c(common_degs_dir,'SCING-M0-hdWGCNA-all'), combine = T, split.by = 'TGFB_inducible',label = T, slot = 'new_scale.data') &
  theme(legend.position="right") &
  # scale_color_gradient2(low="blue",mid="white", high = "red")
  scale_colour_gradientn(limits=c(-4,4),colors = rev(brewer.pal(n = 11, name = "RdBu")))

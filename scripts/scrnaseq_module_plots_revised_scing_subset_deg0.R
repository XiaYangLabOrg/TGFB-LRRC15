...
method <- 'deg_0$' # hdWGCNA_M, hdWGCNA_all, deg_0$, degree
outfile_suffix=paste0('with_SCING_',gsub('$','',method, fixed = T),'_subsets')
scing0_subset <- scing0_subset[,grep(method,colnames(scing0_subset)),F]
...
biplot(scing_metacell_pca, x='PC1', y='PC2',lab=NULL, colby="TGFB_inducible", legendPosition = 'right', showLoadings = T)
...
p <- FeaturePlot(scing_metacell, features=c(common_degs_dir), combine = T,label = F, reduction = 'pca', split.by = 'TGFB_inducible', slot = 'new_scale.data') &
  theme(legend.position="right")&
  # scale_color_gradient2(low="blue",mid="white", high = "red")
  scale_colour_gradientn(limits=c(-4,4),colors = rev(brewer.pal(n = 11, name = "RdBu")))
save_plot(paste0("../scRNAseq_analysis/module_umaps/SCING_module_umaps_metacells_",outfile_suffix,".pdf"), p, base_width = 10, base_height=10)
...
p <- FeaturePlot(scing_obj, features=c(common_degs_dir), combine = T, split.by = 'TGFB_inducible',label = F, slot = 'scale.data', pt.size = 1) +
  ...
  theme(legend.position="right", axis.text = element_text(size = 20), axis.title = element_text(size=20), legend.text = element_text(size=20), legend.key.height = unit(4,'cm'), legend.key.width = unit(1,'cm')) &
  scale_color_gradient2(low="#377EB8",mid="white", high = "#E41A1C",limits=c(-4,4), oob = scales::squish)
save_plot(paste0("../scRNAseq_analysis/module_umaps/SCING_module_umaps_singlecells",outfile_suffix,".pdf"), p, base_width = 10, base_height=10)
...
keep_rows <- rownames(comb_plot_df)[rowSums(comb_plot_df[,grepl('p_val_adj', colnames(comb_plot_df))] <=0.05) >=4]
column_split_vals <- column_split_vals[colnames(value_df)]
pdf(paste0('../scRNAseq_analysis/module_umaps/SCING_hdWGCNA_combined_samplewise_ME_deg_scaled_exp_heatmap_',outfile_suffix,'.pdf'), width=8, height=10)
heatmap_width <- 8
...
saveRDS(list(comb_plot_df=comb_plot_df, value_df=value_df, column_split_vals=column_split_vals), "./scrnaseq_heatmap_deg0.rds")
